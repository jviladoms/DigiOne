...
void DMRCodec::process_udp()
{
	...
	if((m_status != CONNECTED_RW) && (::memcmp(buf.data(), "RPTACK", 6U) == 0)){
        ...
        case DMR_OPTS:
            ...
            connect(m_ping_timer, SIGNAL(timeout()), this, SLOT(send_ping()));
            m_ping_timer->start(5000);
            qDebug() << "m_vocoder: " << m_vocoder;
            qDebug() << "m_hwrx: " << m_hwrx;
            qDebug() << "m_hwtx: " << m_hwtx;
            if(m_vocoder != ""){
                qDebug() << "changing hwtx";
                m_hwrx = true;
                m_hwtx = true;
                m_ambedev = new SerialAMBE("DMR");
                m_ambedev->connect_to_serial(m_vocoder);
...
void DMRCodec::tx_management(){
    qDebug() << "tx management";
    if(m_tx == false){
        qDebug() << "tx false";
        start_tx();
    } else{
...
void DMRCodec::build_frame()
{
    ...
    qDebug() << "m_dmrid: " << m_dmrid;
    qDebug() << "m_txdstid: " << m_txdstid;
    qDebug() << "m_essid: " << m_essid;

	m_dmrFrame[15U] = (m_slot == 1U) ? 0x00U : 0x80U;
    qDebug() << "m_flco: " << m_flco;
    qDebug() << "m_flco == FLCO_GROUP " << (m_flco == FLCO_GROUP);
